...
void calibrateAcc() {
  ...
  int16_t accXbuffer[25], accZbuffer[25];
  for (uint8_t i = 0; i < 25; i++) {
    while (i2cRead(accAddress, 0x32, i2cBuffer, 6));
    accXbuffer[i] = (i2cBuffer[0] | (i2cBuffer[1] << 8));
    accZbuffer[i] = (i2cBuffer[4] | (i2cBuffer[5] << 8));
    delay(10);
  }
  if (!checkMinMax(accXbuffer, 25, 1000) || !checkMinMax(accZbuffer, 25, 1000)) {
    Serial.print(F("Accelerometer calibration error"));
    while (1); // Halt
  }
  for (uint8_t i = 0; i < 25; i++) {
    cfg.accXzero += accXbuffer[i];
    cfg.accZzero += accZbuffer[i];
  }
  cfg.accXzero /= 25.0f;
  cfg.accZzero /= 25.0f;

  if (cfg.accXzero < 0) // Check which side is laying down
    cfg.accXzero += 16384.0f; // 16384.0 is equal to 1g while the full scale range is ±2g
  else
    cfg.accXzero -= 16384.0f;
    
  Serial.print(F("New zero values (g's): ")); // Print the new values in g's
  Serial.print(cfg.accXzero/16384.0f);
  Serial.print(F(","));
...
bool calibrateGyro() {
  int16_t gyroYbuffer[25];
    for (uint8_t i = 0; i < 25; i++) {
      ...
      gyroYbuffer[i] = ((i2cBuffer[2] << 8) | i2cBuffer[3]);
      delay(10);
    }
    if (!checkMinMax(gyroYbuffer, 25, 200)) {
